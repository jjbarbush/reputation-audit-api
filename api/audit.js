...
async function runSearch(query) {
    var response = await client.messages.create({
          ...
          messages: [{ role: 'user', content: 'Search the web for: "' + query + '". Summarize all the key findings, mentions, reviews, complaints, and sentiments you find. Include specific quotes, ratings, platform names, and URLs where possible. Be thorough and factual.' }]
    });
    var text = '';
...
export default async function handler(req, res) {
  ...
  try {
      ...
      if (!data) {
              return res.status(422).json({ success: false, error: 'Failed to parse analysis results', raw_preview: rawText.substring(0, 300) });
      }
